...
-- nNTILE 균등하게 나누니 NTILE(4) 4등분
-- NTILE(4) OVER (ORDER BY 총구매금액) AS 4분위 -- 1,2,3,4
...
customer_grade AS (
	SELECT
		...
		총구매금액,
		구매횟수,
		NTILE(4) OVER (ORDER BY 총구매금액) AS 분위4, -- 총구매금액을 기준으로 정렬해서 4등분(1~4) 즉, 1은 하위 25%, 4는 상위 25% 그룹
...
SELECT
	...
	category,
	price,
	RANK() OVER (ORDER BY price) AS 가격순위, -- 가격이 저렴한 순서대로 순위를 매김
	PERCENT_RANK() OVER (ORDER BY price) AS 백분위순위,
	CASE
		WHEN PERCENT_RANK() OVER (ORDER BY price) <= 0.9 THEN '최고가(상위10%)'
		WHEN PERCENT_RANK() OVER (ORDER BY price) <= 0.7 THEN '최고가(상위30%)'
		WHEN PERCENT_RANK() OVER (ORDER BY price) <= 0.3 THEN '최고가(상위70%)'
		ELSE '저가(하위30%)'
	END AS 가격등급
FROM products;

--
-- 최고/최저(파티션에서의 최고와 최저를 찾는 window함수)
--
